...
export function recordHypotheticalExit(portfolio, price) {
  if (!portfolio.position) return { portfolio, event: null };

  const proceeds = portfolio.position.quantity * price;
  const entryValue = portfolio.position.quantity * portfolio.position.entry;
  const trade = {
    side: "SELL",
    price,
    ...
    reason: "Manual hypothetical exit"
  };
